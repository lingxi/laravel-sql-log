...
namespace Lingxi\LaravelSqlLogger;
...
class SqlLogger
{
    ...
    public function __construct($app)
    {
        ...
        $this->override           = array_get($config, 'override_log');
        $this->directory          = rtrim(array_get($config, 'directory'), '\\/');
        $this->convertToSeconds   = array_get($config, 'convert_to_seconds');
        $this->separateConsoleLog = array_get($config, 'log_console_to_separate_file');
    }
